...
## For another explanation of dplyr see the dplyr package vignette: 
## Introduction to dplyr
## Why is it useful?

## The package contains a set of functions (or “verbs”) that perform 
## common data manipulation operations such as filtering for rows, 
...
## another common task which is the “split-apply-combine” concept. 
...
## such as split(), subset(), apply(), sapply(), 
## lapply(), tapply() and aggregate(). Compared to base functions in R, 
## the functions in dplyr are easier to work with, 
## are more consistent in the syntax and are targeted for data analysis 
## around data frames instead of just vectors.
...
## Data: mammals sleep
## The msleep (mammals sleep) data set contains the sleeptimes and 
## weights for a set of mammals and is available in the dagdata 
## repository on github. This data set contains 83 rows and 11 variables.
...
#The two most basic functions are select() and filter() which selects columns 
# and filters rows, respectively.
...
## You can use the boolean operators (e.g. >, <, >=, <=, !=, %in%) 
## to create the logical tests.

## Pipe operator: %>%

# Before we go any futher, let’s introduce the pipe operator: %>%. 
# dplyr imports this operator from another package (magrittr). 
# This operator allows you to pipe the output from one function 
...
# that will select two columns (name and sleep_total) and 
# then pipe the new data frame to the function head() 
# which will return the head of the new data frame.

msleep %>% 
  select(name, sleep_total) %>% 
  head

# You will soon see how useful the pipe operator is when 
# we start to combine many functions.
...
# such as the taxonomic order, list the name of the column
# you want to arrange the rows by

msleep %>% arrange(order) %>% head


# Now, we will select three columns from msleep, arrange the rows 
...
# Same as above, except here we filter the rows for mammals 
# that sleep for 16 or more hours instead of showing
# the head of the final data frame
...
# Something slightly more complicated: same as above, 
# except arrange the rows in the sleep_total column in a descending order. 
...
# Create a new column called rem_proportion which 
# is the ratio of rem sleep to total amount of sleep.

msleep %>% 
  mutate(rem_proportion = sleep_rem / sleep_total) %>%
  head

# You can make many new columns using mutate (separated by commas). 
# Here we add a second column called bodywt_grams 
# which is the bodywt column in grams.
...
# The summarise() function will create summary statistics 
# for a given column in the data frame such as finding the mean. 
# For example, to compute the average number 
# of hours of sleep, apply the mean() function to the column sleep_total and 
# call the summary value avg_sleep.
...
# We literally want to split the data frame by some variable (e.g. taxonomic order), 
...
# between x and y, all combination of the matches are returned. This is a mutating join.
...
# The join result has all variables from x = superheroes plus  yr_founded, from y.
...
# We basically get x = superheroes back, 
## but with the addition of variable yr_founded, 
## which is unique to  y = publishers. 
## Hellboy, whose publisher does not appear in y = publishers, 
## has an NA for  yr_founded.
